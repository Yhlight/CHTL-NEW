...
bool CompilerDispatcher::Compile(const std::string& sourceCode) {
    ...
    try {
        ...
        if (m_FragmentsByType.count(FragmentType::CHTL_FRAGMENT) > 0) {
            ...
            std::cout << "✅ CHTL片段编译完成，HTML长度: " << result.Content.length() << std::endl;
        }
...
CompilationResult CompilerDispatcher::CompileCHTLFragments(const std::vector<CodeFragment>& fragments) {
    ...
    try {
        ...
        for (const auto& fragment : fragments) {
            std::cout << "   🔍 处理CHTL片段，内容长度: " << fragment.Content.length() << std::endl;
            std::cout << "   📝 片段内容预览: " << fragment.Content.substr(0, 100) << "..." << std::endl;
            ...
            std::cout << "   📋 解析结果: " << (parseResult.IsSuccess ? "成功" : "失败") << std::endl;
            
            if (parseResult.IsSuccess && parseResult.RootNode) {
                std::cout << "   🏗️ AST根节点存在，开始生成..." << std::endl;
                // 使用生成器生成真正的HTML、CSS、JS
                auto genResult = m_CHTLGenerator->Generate(std::move(parseResult.RootNode));
                std::cout << "   📊 生成结果: " << (genResult.IsSuccess ? "成功" : "失败") << std::endl;
                
                if (genResult.IsSuccess) {
                    std::cout << "   📄 HTML内容长度: " << genResult.HTMLContent.length() << std::endl;
                    std::cout << "   🎨 CSS内容长度: " << genResult.CSSContent.length() << std::endl;
                    std::cout << "   ⚡ JS内容长度: " << genResult.JavaScriptContent.length() << std::endl;
